...
$currentPath = isset($_SERVER['REQUEST_URI']) ? esc_url_raw((string) wp_unslash($_SERVER['REQUEST_URI'])) : '/comparisons/';
$redirectTo = wp_validate_redirect(home_url($currentPath), home_url('/'));
?>

<main class="mm-comparisons container mm-page mm-page-980">
    <h1><?php echo 'Comparison history'; ?></h1>

    <?php if (!is_user_logged_in()) : ?>
        <p><?php echo 'Please log in to see your comparison history.'; ?></p>
        <p>
    ...
    <?php else : ?>
        ...
        <?php if ($rows === []) : ?>
            <div class="mm-empty mm-mt-md">
                <p><?php echo 'No comparisons yet. Take a quiz and share the compare link to get started.'; ?></p>
            </div>
        <?php else : ?>
                        ...
                        <div class="mm-compare-card-top">
                            ...
                            <div class="mm-compare-meta">
                                <div class="mm-compare-line">
                                    ...
                                    <?php if ($ago !== '') : ?>
                                        <?php echo 'did a comparison'; ?> <?php echo esc_html($ago); ?>
                                    <?php else : ?>
                                        <?php echo 'did a comparison recently'; ?>
                                    <?php endif; ?>
                                </div>
                                <div class="mm-compare-sub"><?php echo esc_html($quizTitle); ?> • <?php echo esc_html($matchScore); ?>% <?php echo 'match'; ?></div>
                            </div>
                        </div>
                        <div class="mm-compare-actions">
                            <a class="mm-compare-link" href="<?php echo esc_url($url); ?>"><?php echo 'Check results'; ?></a>
                        </div>
                    </article>
                <?php endforeach; ?>
            </div>
